...
class SnakeFCN(torch.nn.Module):
    def __init__(self, numActions, locPoint):
        
        super().__init__()

        self.LOC_F1 = torch.nn.Sequential(collections.OrderedDict([
            ('f', torch.nn.Linear(locPoint, 256)),
            ('ReLU', torch.nn.ReLU()),
            # ('dropout', torch.nn.Dropout(p=0.5))
        ]))
